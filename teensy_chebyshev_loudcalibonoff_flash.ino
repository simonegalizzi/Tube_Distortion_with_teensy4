...
AudioFilterBiquad loudnessFilterL;
AudioFilterBiquad loudnessFilterR;
...
AudioConnection patchLoudInL(inputGainL, 0, loudnessFilterL, 0);
AudioConnection patchLoudOutL(loudnessFilterL, 0, bypassMixerL, 1); // Direct + Loudness
...
AudioConnection patchLoudInR(inputGainR, 0, loudnessFilterR, 0);
AudioConnection patchLoudOutR(loudnessFilterR, 0, bypassMixerR, 1);
...
bool bypassEnabled = true;  // Stato bypass
...
int moltiplica_delay = 1; // 1 10ms 2 100 ms 3 1000 ms
// x calibrazione
bool autoCalibEnabled = true; // variabile runtime per flash

bool loudnessEnabled = true;
float loudnessGainDB = 6.0f;              // ora regolabile da menu
const float LOUDNESS_TREBLE_RATIO = 0.66f; // gli acuti seguono i bassi in proporzione
const float LOUDNESS_GAIN_MIN = 0.0f;
const float LOUDNESS_GAIN_MAX = 15.0f;
//const float LOUDNESS_FREQ = 150.0f;
//const float LOUDNESS_HIGH_FREQ = 6000.0f;

float loudnessFreq = 150.0f;        // era const LOUDNESS_FREQ
float loudnessHighFreq = 6000.0f;   // era const LOUDNESS_HIGH_FREQ
const float LOUDNESS_FREQ_MIN = 40.0f;
const float LOUDNESS_FREQ_MAX = 400.0f;
const float LOUDNESS_HIGH_FREQ_MIN = 2000.0f;
const float LOUDNESS_HIGH_FREQ_MAX = 12000.0f;
...
enum MenuState {
  ...
  MENU_VIEW_PRESET,   //12
  MENU_SAVE_PRESET,   //13
  MENU_TOGGLE_LOUDNESS,//14
  MENU_EDIT_LOUDNESS_GAIN, //15 regolazione dB
  MENU_EDIT_LOUDNESS_FREQ,//16
  MENU_EDIT_LOUDNESS_HIGHFREQ,//17
  MENU_TOGGLE_AUTOCALIB,//18
  MENU_SPEGNI         //19
};

MenuState currentMenu = MENU_MAIN;
int menuSelection = 0;
const int numMenuItems = 19;  
       ...
       case MENU_DELAY: {
        ...
        case MENU_VIEW_PRESET: {
         ...
         case MENU_EDIT_LOUDNESS_GAIN:{
            loudnessGainDB += delta * 0.5f;
            if (loudnessGainDB < LOUDNESS_GAIN_MIN) loudnessGainDB = LOUDNESS_GAIN_MIN;
            if (loudnessGainDB > LOUDNESS_GAIN_MAX) loudnessGainDB = LOUDNESS_GAIN_MAX;
            updateLoudness();
          break;
         }
         case MENU_EDIT_LOUDNESS_FREQ:{
            loudnessFreq += delta * 10.0f;
            if (loudnessFreq < LOUDNESS_FREQ_MIN) loudnessFreq = LOUDNESS_FREQ_MIN;
            if (loudnessFreq > LOUDNESS_FREQ_MAX) loudnessFreq = LOUDNESS_FREQ_MAX;
            updateLoudness();
          break;
         }
         case MENU_EDIT_LOUDNESS_HIGHFREQ:{
            loudnessHighFreq += delta * 250.0f;
            if (loudnessHighFreq < LOUDNESS_HIGH_FREQ_MIN) loudnessHighFreq = LOUDNESS_HIGH_FREQ_MIN;
            if (loudnessHighFreq > LOUDNESS_HIGH_FREQ_MAX) loudnessHighFreq = LOUDNESS_HIGH_FREQ_MAX;
            updateLoudness();
          break;
         }
...
void handleButtons() {
  ...
  if (confirmState == LOW && lastConfirmState == HIGH) {
    if (currentMenu == MENU_MAIN) {
      switch (menuSelection) {
        ...
        case 13: currentMenu = MENU_TOGGLE_LOUDNESS; break;
        case 14: currentMenu = MENU_EDIT_LOUDNESS_GAIN; break;
        case 15: currentMenu = MENU_EDIT_LOUDNESS_FREQ; break;
        case 16: currentMenu = MENU_EDIT_LOUDNESS_HIGHFREQ; break;
        case 17: currentMenu = MENU_TOGGLE_AUTOCALIB; break;
        case 18: currentMenu = MENU_SPEGNI; 
  ...
  if (currentMenu == MENU_DELAY && pushState == LOW) {
        moltiplica_delay++;
        if (moltiplica_delay > 3) moltiplica_delay = 1;
        switch (moltiplica_delay) {
  
        case 1: DELAY_STEP=10; break;
        case 2: DELAY_STEP=100; break;
        case 3: DELAY_STEP=1000; break;
        }
        updateDisplay();
        delay(200);
       return;
...
void setup() {
 ...
 // erase_flash_log();
  ...
  //Serial.println("autocalib "+String(storage.autoCalibEnabled));
  ...
  delay(200);
  AudioMemory(190);  // Aumentato per i mixer aggiuntivi
  ...
  setupFilters();
  updateWetDryMix();
  updateBypass();  // Imposta bypass iniziale (ON)
  updateLoudness();
 ...
 // Serial.println(String(autoCalibEnabled));
  if (autoCalibEnabled) {
  display.clearDisplay();
